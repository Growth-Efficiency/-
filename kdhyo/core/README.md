...
- 스프링 빈을 등록하는 방법은 직접 등록하는 방법(xml)과 빈 팩토리를 통해서 등록하는 방법이 있다.
- 일반적으로 자바 코드로 등록하는 방법은 빈 팩토리를 통하는 방법이다.
- 다형성 + OCP, DIP 를 가능하게 하는 기술
  - DI (Dependency Injection) : 의존관계, 의존성 주입
  - DI 컨테이너 : 의존관계를 연결해주는 컨테이너
- 클라이언트 코드의 변경 없이 기능 확장
...
- 종료 메서드 추론
  - @Bean 의 destroyMethod 속성에는 아주 특별한 기능이 있따.
...
- 스프링에 종속적인 기술이 아닌 JSR-250 자바 표준. 스프링이 아닌 다른 컨테이너에서도 동작가능하다.
- 컴포넌트 스캔과 잘 어울린다.
- 단점
  - 외부 라이브러리에는 적용하지 못한다.
  - 외부 라이브러리를 초기화, 종료 해야 한다면 @Bean 의 기능을 사용하자.

## 빈 스코프

> 빈이 존재할 수 있는 범위를 뜻한다. 기본적으로 싱글톤 스코프로 생성된다.
...
- 그 후에는 요청한 클라이언트에게 빈을 반환한 후 해당 빈을 컨테이너에서 관리하지 않는다.
...
- request: 웹 요청이 들어오고 나갈 때까지 유지되는 스코프
  - ObjectProvider 를 통해 호출하는 시점에 빈을 생성할 수 있다.
  - @Scope 에 proxyMode = `ScopedProxyMode.TARGET_CLASS` 를 추가하면 프록시 객체를 만든다.
    - 가짜 프록시 객체는 요청이 오면 그때 내부에서 진짜 빈을 요청하는 위임 로직이 존재한다.
    - 클라이언트가 `myLogger.logic()` 을 호출하면 프록시 객체의 메서드를 호출한 것이다.
    - 프록시 객체는 request 스코프의 진짜 `myLogger.logic()`를 호출한다.
    - 프록시 객체는 원본 클래스를 상속받아 만들었기 때문에 이 객체를 사용하는 클라이언트 입장에서는  
      사실 원본인지 아닌지도 모르게, 동일하게 사용할 수 있다. (다형성)
  - 동작원리
    - CGLIB 라는 라이브러리로 내 클래스를 상속 받은 프록시 객체를 만들어 주입한다.
    - 프록시 객체는 실제 요청이 오면 그때 내부에서 실제 빈을 요청하는 위임 로직이 들어있다.
    - 프록시 객체는 실제 request scope 와 관계가 없고, 단순 위임 로직만 있어 싱글톤처럼 동작한다.
  - 특징
    - 프록시 객체를 통해 마치 싱글톤 빈을 사용하듯 편리하게 request scope 를 사용할 수 있다.
    - Provider, 프록시 어떤 걸 사용하든 핵심 아이디어는 진짜 객체를 조회를 꼭 필요한 시점까지 지연처리하는 점
    - 애노테이션 설정 변경만으로 원본 객체를 프록시 객체로 대체할 수 있다.
    - 이것이 바로 다형성과 DI 컨테이너가 가진 큰 장점이다.
    - 꼭 웹 스코프가 아니어도 프록시는 사용할 수 있다.
  - 주의점
    - 싱글톤처럼 동작하는 것 같지만, 다르게 동작하기 때문에 주의해서 사용해야 한다.
    - 이런 특별한 scope 는 꼭 필요한 곳에서만 최소화해서 사용하자.
